(** * 6.887 Formal Reasoning About Programs, Spring 2016 - Pset 10 *)
...
(* This predicate captures when a command is through executing. *)
...
Module Type S.
  (* Finally, we prove deadlock-freedom: if all commands are good citizens, then
   * execution never reaches a state where not all threads are finished, but where
   * it also is not possible to take a step. *)
